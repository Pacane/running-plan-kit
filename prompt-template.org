...
- Starting from scratch, or less than ~2 months of regular running right now?
  (if yes, the plan starts with a walk/run on-ramp — §15): »… «
...
4. *Build the progression.* If the on-ramp rule (§15) applies, phase one of the
   plan IS Programme fractionné 1 — schedule it explicitly with its exit
   criterion, and start week 1 of the volume build only after it. Then apply
